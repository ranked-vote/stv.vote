<script lang="ts">
  ...
  function getTooltipText(contest: { interesting: boolean; winnerNotFirstRoundLeader: boolean; condorcetWinner?: string }): string | null {
    if (contest.interesting && contest.winnerNotFirstRoundLeader) {
      ...
      return `This election is highlighted because one of the following:<br>• The STV winner differs from the Condorcet winner (the candidate who would beat all others in head-to-head matchups)<br>• There is a Condorcet cycle (no single candidate beats all others)<br>• Exhausted ballots outnumber the winner's votes`;
