/**
 * STV (Single Transferable Vote) Tabulator
 * 
 * Implements the Cambridge, MA variant:
 * - Droop quota: floor(votes / (seats + 1)) + 1
 * - Whole vote surplus transfers (Gregory method)
 * - Random ballot selection for surplus (we use deterministic order for reproducibility)
 */
 ...
 */
export function calculateDroopQuota(totalBallots: number, seats: number): number {
...
export function tabulateSTV(
  ...
  const candidateStatus = new Map<string, 'active' | 'elected' | 'eliminated'>();
  ...
  while (winners.length < seats && roundNumber <= maxRounds) {
        ...
        if (surplus > 0 && winners.length < seats) {
          ...
          for (const ballot of transferBallots) {
            ...
            if (nextChoice) {
              ...
              transferCounts.set(nextChoice, (transferCounts.get(nextChoice) || 0) + ballot.weight);
            } else {
              exhaustedBallots.push(ballot);
              transferCounts.set('X', (transferCounts.get('X') || 0) + ballot.weight);
            }
          }
    ...
    rounds.push({
      ...
      continuingBallots,
      transfers,
      electedThisRound: electedThisRound.length > 0 ? electedThisRound : undefined,
      eliminatedThisRound: eliminatedThisRound.length > 0 ? eliminatedThisRound : undefined,
    });
    
    roundNumber++;
    
    // Check if remaining active candidates can fill remaining seats
    const remaining = Array.from(candidateMap.values()).filter(c => c.status === 'active');
  ...
  const candidateVotes: ICandidateVotes[] = Array.from(candidateMap.values()).map(c => ({
    candidate: c.index,
...
if (import.meta.main) {
  ...
  console.log(`Winners: ${result.winners.map(w => result.candidates[w]).join(', ')}`);
  ...
  for (let i = 0; i < result.rounds.length; i++) {
    ...
    for (const alloc of round.allocations) {
      const name = alloc.allocatee === 'X' ? 'Exhausted' : result.candidates[alloc.allocatee];
      console.log(`  ${name}: ${alloc.votes}`);
    }
    if (round.electedThisRound?.length) {
      console.log(`Elected: ${round.electedThisRound.map(w => result.candidates[w]).join(', ')}`);
    }
    if (round.eliminatedThisRound?.length) {
      console.log(`Eliminated: ${round.eliminatedThisRound.map(w => result.candidates[w]).join(', ')}`);
    }
